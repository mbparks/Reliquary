(()=>{
  ...
  function guideHtml(g){
    return `<section class="panel module-guide" data-module-guide="1"><div class="head"><div><span class="tag">${g.tag}</span><h2>${g.title}</h2><p class="lede">This module answers: <b>${g.answers}</b></p></div></div><div class="cards"><div class="card good"><h3>Use this for</h3><p>${g.use}</p></div><div class="card gap"><h3>Do not use this for</h3><p>${g.not}</p></div><div class="card"><h3>Good next actions</h3><ul>${g.prompts.map(p=>`<li>${p}</li>`).join('')}</ul></div></div></section>`;
  }
  function patchNav(){
  ...
  function patchVersionLabels(){
    document.querySelectorAll('.tag,.pill').forEach(el=>{
      if(/v5\.2\.[01]/.test(el.textContent)||/v5\.2\.0/.test(el.textContent))el.textContent=el.textContent.replace(/v5\.2\.[01]|v5\.2\.0/g,'v5.3.0');
    });
  }
  function enhance(){
    patchNav();
